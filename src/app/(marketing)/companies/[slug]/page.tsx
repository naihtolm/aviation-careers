...
export default async function CompanyDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-start justify-between gap-4">
          ...
          <div>
            <h1 className="font-display text-2xl font-semibold text-white">
              ...
              {company.verification_status === "approved" && (
                <span className="ml-2 text-sm text-brand-300 align-middle" title="Verified employer">✓ Verified</span>
              )}
            </h1>
            <p className="text-slate-400 mt-1">{companyTypeLabel(company.company_type)}</p>
            {company.veteran_friendly && (
              <span className="inline-flex items-center gap-1 text-xs font-medium bg-white/10 text-slate-300 px-2 py-0.5 rounded-full mt-1.5">
        ...
        {company.website && (
          <a
            ...
            className="inline-flex items-center gap-1.5 text-sm border border-white/15 rounded-md px-3 py-1.5 text-slate-300 hover:border-brand-300 hover:bg-white/5 hover:-translate-y-0.5 transition-all shrink-0"
          >
            Visit website
      ...
      {company.description && <p className="text-slate-300 mt-4 max-w-2xl">{company.description}</p>}
        ...
        <Tabs
          tabs={[
            ...
            },
            {
              ...
              content:
                jobs.length === 0 ? (
                  <p className="text-sm text-slate-400">No open jobs right now.</p>
                ) : (
                  <div className="grid sm:grid-cols-2 gap-3">
                    {jobs.map((j: any) => (
                      <Link key={j.id} href={`/jobs/${j.slug}`} className="border border-white/10 rounded-lg p-3 bg-white/[0.04] hover:border-brand-300 hover:bg-white/[0.07] hover:-translate-y-0.5 transition-all">
                        ...
                        <p className="text-xs text-slate-400 mt-0.5 capitalize">{j.employment_type?.replace("_", " ")}</p>
                      </Link>
                    ))}
                  </div>
                ),
                  ...
                  <ul className="space-y-1">
                    {airportLinks.map((a: any, i: number) => (
                      <li key={i} className="text-sm">
                        <Link href={`/airports/${a.airports?.iata_code}`} className="text-brand-300 hover:underline hover:text-brand-200 transition-colors">
                          {a.airports?.name} ({a.airports?.iata_code})
                        </Link>
                        <RelationshipBadge type={a.relationship_type} className="text-slate-500 ml-2" />
                      </li>
                    ))}
                  </ul>
                ),
